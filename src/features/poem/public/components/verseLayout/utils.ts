...
export function calculateWidth1(averageLengthText: number, fontSize: TFontSize) {
    const widthClass =
...
export function calculateWidth2(averageLengthText: number, fontSize: TFontSize) {
    const widthClass =
        averageLengthText > 32
            ? {
            ...
            : averageLengthText > 27 ? {
                ...
                large: "*:w-60 *:xs:w-60 *:xs2:w-85 *:md:w-105",
                medium: "*:w-62 *:xs:w-62 *:xs2:w-75 *:md:w-95",
                small: "*:w-50 *:xs:w-55 *:xs2:w-62 *:md:w-65",
            }[fontSize]
    return widthClass
}
